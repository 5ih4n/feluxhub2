<x-app-layout>
    <div class="max-w-2xl mx-auto p-4 sm:p-6 lg:p-8">
        <form method="POST" action="{{ route('feluxvideos.store') }}">
            @csrf
            <textarea
                name="message"
        ...
        <div class="mt-6">
            @foreach ($feluxvideos as $feluxvideo)
                    ...
                    <div class="flex-1">
                        <div class="flex justify-between item-center">
                            <div>
                                ...
                                @unless ($feluxvideo->created_at->eq($feluxvideo->updated_at))
                                    <small class="text-sm text-gray-400"> &middot; {{__('edited')}}</small>
                                @endunless
                            </div>
                            @if ($feluxvideo->user->is(auth()->user()))
                                <x-dropdown>
                                    <x-slot name="trigger">
                                        <button>
